...
新しいバックエンドAPIを作成する際は、実装より先に仕様を確定させる:

1. API仕様(エンドポイント・パラメータ・レスポンス/エラースキーマ・制約事項)をMarkdownとして作成し、提示する。この時点では実装コードは書かない。
2. 内容に問題なしと確認が得られたら、そのMDを`backend/docs/api/`配下に配置し、その後で実装(ドメインロジック・コントローラー・テスト)に進む。
