...
新しいバックエンドAPI・フロントエンド画面を作成する際は、実装より先に仕様を確定させる:

1. 仕様をMarkdownとして作成し、提示する。この時点では実装コードは書かない。
   - バックエンドAPIの場合: エンドポイント・パラメータ・レスポンス/エラースキーマ・制約事項
   - フロントエンド画面の場合: 画面の目的・入力項目・操作(ボタン等)・表示内容・状態(ローディング/エラー等)
2. 内容に問題なしと確認が得られたら、そのMDを配置し、その後で実装に進む。
   - バックエンドAPI: `backend/docs/api/`配下に配置後、実装(ドメインロジック・コントローラー・テスト)
   - フロントエンド画面: `frontend/docs/screens/`配下に配置後、実装(コンポーネント)
