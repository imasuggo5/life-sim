...
4. 年齢`currentAge`〜`100`について、累積貯蓄額(円)を計算する。各資産は自身の年利で複利成長させ、毎年の収支(収入−支出)は資産の運用とは別に「現金バッファ」として利率なしで積み上げる:
   ...
   - `assetTotal[age] = Σ assetBalance_i[age]`(資産額。資産の複利成長分のみを反映し、累積貯蓄額には含めない)
   - `cumulative[age] = cashBuffer[age]`(累積貯蓄額は現金バッファのみで、資産額は含めない)
5. 表示直前に万円へ変換(`÷ 10,000`)し、横軸=年齢、縦軸=累積貯蓄額(万円、資産を含まない現金バッファのみ)の折れ線グラフとして表示する。グラフには0万円のラインを強調表示する(赤色の基準線)。
6. グラフ上の各点をホバーした際のツールチップに、その年齢の「収入」「支出」「年間収支(収入−支出)」「資産額」「累積貯蓄額」をあわせて表示する。
