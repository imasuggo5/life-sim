...
「現在の貯蓄額」(単一の値)を廃止し、資産をレコード形式で複数登録できるようにする。各レコードは`{ name(名前), amountManYen(金額、万円), annualRatePercent(年利、%) }`を持つ。行の追加・削除が自由にできる(初期状態は1行: 名前=「資産1」、金額=1200万円、年利=0%)。
...
- リストの下に「資産を追加」ボタンを置き、押すと新しい行(名前は「資産N」、Nは追加時点の行数+1、金額0、年利0)を末尾に追加する。
...
4. 年齢`currentAge`〜`100`について、貯金額(円)を計算する。各資産は自身の年利で複利成長させ、毎年の収支(収入−支出)は資産の運用とは別に「現金バッファ」として利率なしで積み上げる:
...
6. グラフ上の各点をホバーした際のツールチップに、その年齢の「収入」「支出」「年間収支(収入−支出)」「資産額」「貯金額」をあわせて表示する。
