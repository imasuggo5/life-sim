...
- 働き方は次の3択のセレクトボックス(制度上の「第1号/第2号/第3号被保険者」という用語は画面に出さず、実生活の状態で表現する):
  - `employee` = 「会社員・公務員(厚生年金あり)」(第2号被保険者に相当)
  - `self_employed` = 「自営業・フリーランス・学生など」(第1号被保険者に相当)
  - `dependent_spouse` = 「配偶者の扶養に入っている(専業主婦・主夫等)」(第3号被保険者に相当)
  - デフォルトは全年代とも`employee`。
- 退職年齢`retirementAge`(歳、0〜100、デフォルト65)は収入セクションに残す。
